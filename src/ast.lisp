...
(defmethod get-location ((node let*-form) location)
  (trivia:cmatch (location-id location)
    ...
    ((list (eql 'vars) (and (type integer) i))
     (nth i (vars node)))
    ((list (eql 'vars) (and (type integer) i) (eql 'binder))
     (let ((b (nth i (vars node))))
       (if (symbolp b) b (car b))))
    ((list (eql 'vars) (and (type integer) i) (eql 'init))
     (let ((b (nth i (vars node))))
       (if (symbolp b) nil (second b))))))
...
(defmethod update ((node let*-form) id new-value)
  (trivia:cmatch id
    ...
    ((list (eql 'vars) (and (type integer) i) (eql 'init))
     (let* ((old-body (vars node))
            (p (nth i old-body))
            (old-binder (if (symbolp p) p (car p))))
       (make-instance 'let*-form :vars (list-update old-body (list old-binder new-value) i)
                                 :decls (decls node)
                                 :body (body node))))
    ((list (eql 'vars) (and (type integer) i) (eql 'binder))
     (let* ((old-body (vars node))
